...
void save_setting_to_file(const char *filepath, Setting_t *setting) {
    FILE *fp = fopen(filepath, "wb");
    if (fp) {
        fwrite(setting, sizeof(Setting_t), 1, fp);
        fclose(fp);
    } else {
        zlog_info(debug_out,"Failed to write setting\r\n");
        printf("Failed to write setting\r\n");
    }
}

int load_setting_from_file(const char *filepath, Setting_t *setting) {
    FILE *fp = fopen(filepath, "rb");
    if (fp) {
        fread(setting, sizeof(Setting_t), 1, fp);
        fclose(fp);
        return 0;
    }
    return -1;
...
void G_settings_init() {
    ...
    if (config_exist && load_setting_from_file(CONFIG_FILE_PATH, &setting) == 0 && setting.flag == 1) {
        struct in_addr addr;
        addr.s_addr = htonl(setting.ip);
        printf("Read IP from config: %s\n", inet_ntoa(addr));
        zlog_info(debug_out,"Read IP from config: %s\n", inet_ntoa(addr));
    } else {
        // 配置文件不存在或数据无效，写入默认值
        setting.flag = 1;
        setting.ip = (192 << 24) | (168 << 16) | (1 << 8) | 110;

        save_setting_to_file(CONFIG_FILE_PATH, &setting);

        struct in_addr addr;
        addr.s_addr = htonl(setting.ip);
        printf("Default IP used and saved: %s\n", inet_ntoa(addr));
        zlog_info(debug_out, "Default IP used and saved: %s\n", inet_ntoa(addr));
    }
}
...
//     if (load_setting_from_file(CONFIG_FILE_PATH, &setting) == 0 && setting.flag == 1) {
//         struct in_addr addr;
//         addr.s_addr = htonl(setting.ip); // 转为网络序
//         printf("Read IP from config: %s\n", inet_ntoa(addr));
//         zlog_info(debug_out,"Read IP from config: %s\n", inet_ntoa(addr));
//     } else {
//         // 默认IP：192.168.1.110
