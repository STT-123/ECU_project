...
//  *      非 0 表示失败
//  */
// bool HAL_can_init(int *fd)
// {
//     zlog_info(debug_out, "开始初始化 CAN...\n");

//     if (!HAL_can_ifconfig_init())
//     {
...
/*bool HAL_can_write(int fd,struct canfd_frame *pFrame)
{
	...
	while(1)
	{
		...
		if (retval>0)
		{
			...
			HAL_can_get_state((const char *)"can1", &canState);
...
bool HAL_can_set_filter(int fd, struct can_filter *__can_filter , int  num)
{
