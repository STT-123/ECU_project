 ...
 * 功能描述：创建CAN网络
 * 输入参数：device：物理设备名称，如"CAN2"
 *         bitrate:速率，如250000（250Kbps)
 * 输出参数：0有效，其他失效
 */
int HAL_can2_ifconfig_up(void)
{
	FILE* fp = NULL;
	int ret = RESULT_FAIL;
	char buf[128]={0};

	snprintf(buf,127,"ifconfig %s down",PORT_CAN2_CAN_BITRATE);
	system(buf);
	snprintf(buf,127,"ip link set %s type can bitrate %d", PORT_CAN2_CAN_BITRATE, PORT_CAN2_CAN_BITRATE);
	system(buf);
	snprintf(buf,127,"ifconfig %s up",PORT_CAN2_CAN_BITRATE);
	system(buf);

	//检查网络是否存在
	snprintf(buf,127,"ifconfig | grep %s",PORT_CAN2_CAN_BITRATE);
	fp = popen(buf, "r");
	memset(buf,0,128);
...
int HAL_can2_band(int *fd)
{
	...
	*fd=socket(PF_CAN, SOCK_RAW, CAN_RAW);
	...
	//指定CAN0设备
	strcpy(m_ifr.ifr_name, PORT_CAN2_CAN_BITRATE);
	ioctl(*fd,SIOCGIFINDEX, &m_ifr);

	//将套接字m_socket与CAN0相绑定
	m_addr.can_family = AF_CAN;
	m_addr.can_ifindex = m_ifr.ifr_ifindex;
	return bind(*fd, (struct sockaddr *)&m_addr, sizeof(m_addr));
}
 ...
 * 输出参数：0有效，其他失效
 */
int HAL_can2_write(int fd,struct canfd_frame *pFrame)
{
	/*int len=write(fd,pFrame,sizeof(struct canfd_frame));
...
//		LOGE("can send data1, retval=%d; len=%d, canState=%d\r\n", retval, len, canState);
//		zlog_info(debug_out,"can send data1, retval=%d; len=%d, canState=%d\r\n", retval, len, canState);
		if (retval>0)
		{
			len=write(fd, pFrame, sizeof(struct canfd_frame));
		HAL_can2_get_state((const char *)"can0", &canState);
//			if(canState != 0)
//			{
//				LOGE("can0 status is %02X\r\n", canState);
//			}
//			LOGE("can send data2, retval=%d; len=%d, canState=%d\r\n", retval, len, canState);
//			zlog_info(debug_out,"can send data2, retval=%d; len=%d, canState=%d\r\n", retval, len, canState);
			if(len == sizeof(struct canfd_frame))
				return RESULT_OK;
			else
				return RESULT_FAIL;
 ...
 * 输出参数：0有效，其他失效
 */
int HAL_can2_read(int fd, struct canfd_frame *pFrame,unsigned int msTimeout)
{
    int retval = 0, len = 0;
    fd_set   fs_read;
...
int HAL_can2_set_filter(int fd, struct can_filter *__can_filter , int  num)
{
...
int HAL_can2_get_state(const char *name, int *state)
{
