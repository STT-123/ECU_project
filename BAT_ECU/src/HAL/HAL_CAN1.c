...
bool HAL_can1_ifconfig_init(void)
{
	...
	snprintf(buf, 127, "ip link set %s down", PORT_CAN1_DEVICE_NAME);
	system(buf);
	// snprintf(buf,127,"ip link set %s type can bitrate %d", PORT_CAN1_DEVICE_NAME, PORT_CAN1_CAN_BITRATE);
	// system(buf);
	snprintf(buf, 127, "ip link set %s type can bitrate %d dbitrate 2000000 fd on", PORT_CAN1_DEVICE_NAME, PORT_CAN1_CAN_BITRATE);
	system(buf);

	snprintf(buf, 127, "ip link set %s up", PORT_CAN1_DEVICE_NAME);
	system(buf);

	//检查网络是否存在
	snprintf(buf,127,"ip link show %s", PORT_CAN1_DEVICE_NAME);
	fp = popen(buf, "r");
...
bool HAL_can1_band_init(int *fd)
{
    ...
    *fd = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    ...
    // 指定 CAN0 设备
    strcpy(m_ifr.ifr_name, PORT_CAN1_DEVICE_NAME);
...
bool HAL_can1_write(int fd,struct canfd_frame *pFrame)
{
	...
	while(1)
	{
		...
		if (retval>0)
		{
			...
			HAL_can1_get_state((const char *)"can0", &canState);
