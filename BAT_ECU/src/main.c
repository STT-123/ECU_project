...
#include "./TASK/Can_MessageRev_Task.h"
#include "./TASK/Net_Config_Task.h"
#include "./TASK/Can_MessageSend_Task.h"
#include "./TASK/Can_MessageSend_Task.h"
#include "./TASK/Log_Save_Task.h"
#include "./TASK/Serial_Led_Task.h"
#include "./TASK/OTA_Upgrad_Task.h"
#include "./TASK/ExternalEmergencyStop_Task.h"
#include "./TASK/SD_CapacityChenkTask.h"
...
static void lws_log_cb(int level, const char *line) ;
// void Fireproofboard_updata();
...
void all_init(void)
{
	...
	Drv_can1_init();
	Drv_can2_init();
	Drv_init_double_ring_buffer(&canDoubleRingBuffer);
	app_zlog_init();
    lws_set_log_level(LLL_ERR | LLL_WARN | LLL_NOTICE | LLL_INFO, lws_log_cb);
	elog_init();
...
void all_thread_init(void)
{
	...
	SDCardDataSaveTaskCreate();

	SerialLedTaskCreate();

	GPIODetectionTaskCreate();
...
static void lws_log_cb(int level, const char *line) {
    switch (level) {
        case LLL_ERR:
            zlog_error(zlog_get_category("debug_file"), "%s", line);
            break;
        case LLL_WARN:
            zlog_warn(zlog_get_category("debug_file"), "%s", line);
            break;
        case LLL_NOTICE:
            zlog_info(zlog_get_category("debug_file"), "%s", line);
            break;
        case LLL_INFO:
            zlog_debug(zlog_get_category("debug_file"), "%s", line);
            break;
        default:
            zlog_debug(zlog_get_category("debug_file"), "%s", line);
            break;
